...
if (!target || !time || !thread || !proxyFile || !rps) {
 console.error('Error: Please provide the following arguments:');
    ...
    console.error(`Example: node ${process.argv[1]} http://example.com/ time threads proxy.txt rps`);
  process.exit(1);
}
...
console.log(`SUCCESS: Attack sent successfully! | Target: ${target} | Duration: ${time} seconds`);
